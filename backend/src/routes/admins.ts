...
// Get current commissioner name (all authenticated admins can view)
router.get('/commissioner', authenticateAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const [admins] = await db.query<RowDataPacket[]>(
    ...
    logger.error('Get commissioner error:', error);
    res.status(500).json({ error: 'Server error' });
  }
});

...
// Exchange magic link token for JWT (new endpoint)
router.post('/exchange/:token', magicLinkValidationLimiter, async (req, res) => {
  ...
  try {
    // First try email-based magic links
    const [emailLinks] = await db.query<RowDataPacket[]>(
      `SELECT eml.*, r.season_id, r.lock_time, r.timezone
       FROM email_magic_links eml
       JOIN rounds_v2 r ON eml.round_id = r.id
       WHERE eml.token = ?`,
      [token]
    ...
    if (emailLinks.length > 0) {
      ...
      // Check if expired (same validation as validate endpoint)
      const now = moment.tz(link.timezone);
      const lockTime = moment.utc(link.lock_time).tz(link.timezone);
      ...
      // Generate JWT for email-based magic link (no userId - will be provided on pick submission)
    ...
    const [links] = await db.query<RowDataPacket[]>(
      `SELECT ml.*, r.season_id, r.lock_time, r.timezone
       FROM magic_links ml
       JOIN rounds_v2 r ON ml.round_id = r.id
       WHERE ml.token = ?`,
      [token]
    ...
    // Generate JWT for user-based magic link
...
// Validate magic link and get round info (updated to use JWT if available)
router.get('/validate/:token', magicLinkValidationLimiter, async (req, res) => {
  const { token } = req.params;

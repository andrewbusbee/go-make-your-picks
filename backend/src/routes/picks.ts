...
/**
 * REMOVED: GET /api/picks/validate/:token
 * 
 * This endpoint was removed for security reasons. Tokens in URL paths can be exposed in:
 * - Browser history
 * - Server access logs
 * - HTTP referrer headers
 * - Proxy/load balancer logs
 * 
 * Use POST /api/picks/validate instead, which accepts the token in the request body.
 * This significantly reduces token exposure risk.
 */
